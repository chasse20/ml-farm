...
class Trainer:
	...
	def Train( self, tThreadCount: int, tTable: Table ) -> TrainedModel:
		if ( tTable is not None ):
			...
			if ( self._hyperParams.HuberDelta > 0 ):
				tempAlpha = self._hyperParams.HuberDelta

			# Train
			tempModel = LGBMRegressor(
			...
			return TrainedModel( tempModel, tempInitialTypes, TrainerType.LGBMRegression )
